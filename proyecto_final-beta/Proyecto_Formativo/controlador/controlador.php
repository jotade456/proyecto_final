...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (
        ...
        if (agregar_producto($nombre, $valor, $imagen)) {
            header("Location: ../ADMIN/index.html?success=1");
        } else {
            header("Location: ../ADMIN/index.html?error=1");
        }
        exit;
    }
...
function obtener_datos_usuario($id_usuario) {
    ...
    if ($usuario && isset($usuario['foto_perfil']) && $usuario['foto_perfil']) {
        if (!file_exists($usuario['foto_perfil'])) {
            $usuario['foto_perfil'] = null; // Imagen eliminada, devolver null
        }
    }
